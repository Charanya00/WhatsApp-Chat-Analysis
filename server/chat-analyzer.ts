...
export async function analyzeChat(content: string): Promise<AnalysisMetrics> {
  ...
  const userStats: UserStats[] = Array.from(userStatsMap.values()).map(u => {
    ...
    const topEmojis = Array.from(u.emojis.entries())
      .map(([emoji, count]: any) => ({ emoji: String(emoji), count: count as number }))
  ...
  const timeline: TimelineData[] = Array.from(timelineMap.entries())
    .map(([date, count]) => ({ date, count, messageCount: count }))
  ...
  for (const t of timeline) {
    if (t.messageCount > busiestDay.count) busiestDay = { date: t.date, count: t.messageCount };
  }
